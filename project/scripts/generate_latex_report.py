...
# 03 system design
summary_rows = [[name, field_list(schema_sql, name), '数据库物理表，与关系模式和后端 Entity 对应'] for name in schema_tables]
chapter03 = r'''
\section{系统设计}
\subsection{本章概述}
...
write(latex / 'README.md', '''# LaTeX 报告说明

本目录为“重庆邮电大学智慧访客预约与出入校管理系统”数据库原理课程设计报告的 LaTeX 项目，使用 `ctexart` 和 XeLaTeX 编译。
...
- `main.tex`：报告主入口，严格按照课程设计要求组织六章内容。
- `chapters/`：系统定义、需求分析、系统设计、详细设计、系统实现与测试、总结。
- \texttt{figures/}：自动截图和后续导出的设计图 PDF/PNG。
- `listings/`：复制保存的 SQL 脚本，便于报告溯源。
- `styles/report-style.tex`：中文、页面、表格、代码块和页眉页脚样式。

## 截图联动

`../scripts/capture_screenshots.js` 会生成 `../screenshots/manifest.json`，并把 19 张截图同步到 \texttt{figures/}。重新截图后请运行 `python ../scripts/generate_latex_report.py` 再编译 PDF。

## 图形导出

Mermaid 或 Graphviz 图源文件位于 `../diagrams/`。如果需要把图插入为 PDF，可安装 Mermaid CLI 或 Graphviz 后执行：

```bash
mmdc -i ../diagrams/system_module.mmd -o figures/system_module.pdf
dot -Tpdf ../diagrams/er_core_business.dot -o figures/er_core_business.pdf
```
''')
